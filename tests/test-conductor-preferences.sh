...
assert_contains 'canonical file has branch rename section' "$PREFS_FILE" '## Branch rename preferences'
assert_contains 'canonical file has create PR section' "$PREFS_FILE" '## Create PR preferences'
assert_contains 'canonical file keeps task branch naming' "$PREFS_FILE" 'task/{id}-{slug}'
assert_contains 'canonical file keeps Track-Task fallback' "$PREFS_FILE" 'Track-Task: {id}'
...
assert_contains 'canonical file tells agent to read task sources first' "$PREFS_FILE" 'Read `TODO.md` and `.track/tasks/` first.'
...
assert_contains 'README includes branch rename block' "$README_FILE" '#### Branch rename preferences'
