...
if diff -u conductor.json skills/init/assets/conductor.json >/tmp/track-conductor-diff 2>&1; then
  pass 'root conductor.json matches asset copy'
else
  fail 'root conductor.json diverges from asset copy'
...
if ! grep -Eq 'custom_prompt_|create_pr|rename_branch|fix_errors|resolve_merge_conflicts' conductor.json && \
   ! grep -Eq 'custom_prompt_|create_pr|rename_branch|fix_errors|resolve_merge_conflicts' skills/init/assets/conductor.json; then
...
if ! grep -Fq 'Track-Task:' conductor.json && \
   ! grep -Fq 'Track-Task:' skills/init/assets/conductor.json; then
