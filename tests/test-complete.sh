...
repo="$(setup_repo)"
run_test "completes a task" 0 bash "$repo/.track/scripts/track-complete.sh" "task/1.1-test-task" "https://github.com/test/pull/1"
...
repo="$(setup_repo)"
run_test "non-task branch is no-op" 0 bash "$repo/.track/scripts/track-complete.sh" "feature/something" ""
rm -rf "$repo"

# Test 3: Already-done task is no-op
repo="$(setup_repo)"
run_test "already-done task is no-op" 0 bash "$repo/.track/scripts/track-complete.sh" "task/1.2-done-task" "https://github.com/test/pull/2"
rm -rf "$repo"
